...
void tft_test_bitmap()
{
    ...
    for (int i = 0; i < 100; i++) {
        for (int j = 0; j < 100; j++) {
            if (j == 10)
                color_buffer[100 * i + j] = 0;
            else
                color_buffer[100 * i + j] = ((r & 0xF8) << 8) | ((g & 0xFC) << 3) | (b >> 3);
            
            if (i == 10) {
                uint8_t tmp_r = 255;
                uint8_t tmp_g = 100;
                uint8_t tmp_b = 0;
                color_buffer[100 * i + j] = tft_color_u16(tmp_r, tmp_g, tmp_b);
            }
        }
    }
    
    __tft_write_pixel_buffer(color_buffer, 100 * 100);
